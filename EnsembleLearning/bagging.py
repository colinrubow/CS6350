...
class Bagging():
    ...
    def bag(self, num_trees: int, directory_train: str, directory_test: str, attributes: dict, outputs: tuple, contains_numeric: bool = False, log: bool = False, num_samples: int = 1000):
        """
        trains and writes a csv of the bag errors.

        num_trees : the number of bagged trees
        ...
        # modify outputs so that outputs[1] is the positive classification and outputs[-1] is the negative classification
        ...
        for _ in range(num_trees):
            ...
            # train the tree
            tree.train_model(None, 'IG', train_data_sampled, attributes, contains_numeric)
            # store the tree
            classifiers.append(tree)
            # loggit
            if log:
                train_error.append(self.get_error(classifiers, train_data, outputs, contains_numeric))
                test_error.append(self.get_error(classifiers, test_data, outputs, contains_numeric))
        # save error data in csv
        if log:
            # prep data
            header = ['train_error', 'test_error']
            error_data = [[train_error[i], test_error[i]] for i in range(len(train_error))]
            with open('./bagging_errors.csv', 'w', newline='') as f:
                writer = csv.writer(f)
